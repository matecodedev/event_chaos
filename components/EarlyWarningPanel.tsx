...
export const EarlyWarningPanel: React.FC<EarlyWarningPanelProps> = ({
  ...
  const visibleWarnings =
    typeof maxItems === 'number' ? warnings.slice(0, Math.max(1, maxItems)) : warnings;

  const getSeverityColor = (severity: 'LOW' | 'MEDIUM' | 'HIGH') => {
    switch (severity) {
      case 'HIGH':
        return 'border-red-500 bg-red-950/80';
      case 'MEDIUM':
        return 'border-orange-500 bg-orange-950/80';
      case 'LOW':
        return 'border-yellow-500 bg-yellow-950/80';
    }
  };
          ...
          <div className="relative z-10">
            <div className="flex items-start gap-2 mb-2">
              ...
              <div className="flex-1">
                ...
                <div
                  className={`text-[10px] font-mono ${SYSTEM_COLORS[warning.systemId].split(' ')[0]}`}
                >
                  {warning.systemId}
                </div>
              </div>
            ...
            <p className="text-xs text-slate-200 mb-2 leading-relaxed">{warning.message}</p>
